...
class EnvironmentValidator {
  ...
  optionalEnvVars: Set<string>;
  foundEnvVars: Set<string>;
  ...
  parseEnvExample(): void {
    ...
    try {
      ...
      this.log(
        `Found ${this.foundEnvVars.size} environment variables (${this.requiredEnvVars.size} required, ${this.optionalEnvVars.size} optional)`,
        "green",
      );
    } catch (error: unknown) {
      const message =
      ...
      this.addIssue(
        "error",
        "config",
        `Failed to parse .env.example: ${message}`,
      );
    }
  }
  ...
  validateEnvVariable(
    ...
    const apiKeyPatterns: Record<string, PatternDef> = {
      OPENAI_API_KEY: {
    ...
    const awsPatterns: Record<string, PatternDef> = {
      AWS_ACCESS_KEY_ID: {
    ...
    const urlPatterns: Record<string, PatternDef> = {
      OLLAMA_BASE_URL: {
        pattern: /^https?:\/\/.+$/,
  ...
  validateSpecificEnvVar(name: string, value: string): void {
    switch (name) {
      case "NODE_ENV":
        if (
          value &&
      ...
      case "LOG_LEVEL":
        if (
          ...
          this.addIssue(
            "warning",
            "value",
            "LOG_LEVEL should be error, warn, info, debug, or trace",
          );
        }
        break;
      ...
      case "DEFAULT_PROVIDER": {
        ...
        if (value && !validProviders.includes(value)) {
          this.addIssue(
            "warning",
            "value",
            `DEFAULT_PROVIDER should be one of: ${validProviders.join(", ")}`,
          );
        }
        break;
      }
  ...
  checkMissingEnvVars(): void {
    this.log(
      "Checking for missing environment variables in codebase...",
      "blue",
    );
    ...
    if (missingFromExample.length > 0) {
      ...
      this.log(
        "All environment variables from code are documented",
        "green",
      );
    }
  }
  ...
  extractEnvVarsFromCode(): Set<string> {
  ...
  validateProviderConfigs(): void {
    ...
    const providerConfigs: Record<string, ProviderConfig> = {
      ...
      "Google AI Studio": {
        ...
        check: () =>
          this.foundEnvVars.has("GOOGLE_AI_API_KEY") ||
          this.foundEnvVars.has("GOOGLE_GENERATIVE_AI_API_KEY"),
      },
      "AWS Bedrock": {
        ...
        check: () =>
          this.foundEnvVars.has("AWS_ACCESS_KEY_ID") &&
          this.foundEnvVars.has("AWS_SECRET_ACCESS_KEY"),
      },
      "Google Vertex AI": {
      ...
      "Hugging Face": {
        ...
        check: () =>
          this.foundEnvVars.has("HUGGINGFACE_API_KEY") ||
          this.foundEnvVars.has("HF_TOKEN"),
      },
      "Mistral AI": {
    ...
    for (const [providerName, config] of Object.entries(providerConfigs)) {
      ...
      if (isConfigured) {
        ...
        const missingOptional = config.optional.filter(
          (varName) => !this.foundEnvVars.has(varName),
        );
    ...
    if (configuredProviders === 0) {
      this.addIssue(
        "warning",
        "providers",
        "No AI providers appear to be configured",
  ...
  checkEnvironmentConsistency(): void {
    ...
    const conflicts = [
      {
        ...
        message:
          "Both GOOGLE_AI_API_KEY and GOOGLE_GENERATIVE_AI_API_KEY are set - use only one",
      },
      {
        vars: ["HUGGINGFACE_API_KEY", "HF_TOKEN"],
        message:
          "Both HUGGINGFACE_API_KEY and HF_TOKEN are set - use only one",
      },
    ];

    for (const conflict of conflicts) {
      const presentVars = conflict.vars.filter((varName) =>
        this.foundEnvVars.has(varName),
      );
    ...
    if (this.foundEnvVars.has("NEUROLINK_EVALUATION_ENABLED")) {
      ...
      const missingEvalVars = evaluationVars.filter(
        (varName) => !this.foundEnvVars.has(varName),
      );
  ...
  printResults(): number {
    ...
    console.log("\n" + "=".repeat(50));
    this.log(`Environment validation completed in ${duration}s`, "cyan");
    ...
    if (this.issues.length > 0) {
      ...
      this.issues.forEach((issue: Issue, index: number) => {
        console.log(
          `${index + 1}. ${colors.red}[${issue.severity.toUpperCase()}]${colors.reset} ${issue.category}: ${issue.message}`,
        );
        if (issue.suggestion) {
    ...
    if (this.warnings.length > 0) {
      console.log(
        `\n${colors.yellow}ENVIRONMENT WARNINGS:${colors.reset}`,
      );
      console.log("=".repeat(50));

      this.warnings.forEach((warning: Issue, index: number) => {
        console.log(
    ...
    if (this.issues.length === 0 && this.warnings.length === 0) {
      ...
      console.log("=".repeat(50));
      console.log(
        `${colors.green}Environment configuration is valid${colors.reset}`,
      );
      console.log(
        `${colors.green}.env.example is complete and accurate${colors.reset}`,
      );
      console.log(
        `${colors.green}Provider configurations look good${colors.reset}`,
      );
      console.log(
        `${colors.green}No consistency issues found${colors.reset}`,
      );
    } else {
      console.log(
    ...
    if (this.issues.length > 0) {
      ...
      console.log(
        `${colors.red}Please address ${this.issues.length} critical environment issues before proceeding.${colors.reset}`,
      );
      return 1;
    } else {
      console.log(
...
// Run the environment validator when executed directly
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  validator
    ...
    .catch((error: unknown) => {
      ...
      console.error(
        `${colors.red}Environment validation crashed: ${message}${colors.reset}`,
      );
      process.exit(1);
    });
}
